...
SmoothedData1 = BilateralFilter(Data, 2.5, 3.0)
SmoothedData2 = BilateralFilter(SmoothedData1, 2.5, 3.0)

while np.sum((SmoothedData1 - SmoothedData2)**2) > 0.01*np.sum(SmoothedData2**2):
    SmoothedData1 = BilateralFilter(SmoothedData2, 2.5, 3.0)
    SmoothedData2 = BilateralFilter(SmoothedData1, 2.5, 3.0)
...
fig.suptitle('Real Data Time Series 3 \n Bilateral Filter - $\sigma_d = 2.5$ - $\sigma_i = 3.0 \hat{\sigma}_n$',  fontsize = 12)
...
plt.savefig('ItrSameBilateralRealSignal3.png')
