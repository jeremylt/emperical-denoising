...
SmoothedData1 = BilateralFilter(Data, 3.3, 2.6)
SmoothedData2 = BilateralFilter(SmoothedData1, 3.3, 2.6)

while np.sum((SmoothedData1 - SmoothedData2)**2) > 0.01*np.sum(SmoothedData2**2):
    SmoothedData1 = BilateralFilter(SmoothedData2, 3.3, 2.6)
    SmoothedData2 = BilateralFilter(SmoothedData1, 3.3, 2.6)
...
fig.suptitle('Real Data Time Series 1 \n Bilateral Filter - $\sigma_d = 3.3$ - $\sigma_i = 2.6 \hat{\sigma}_n$',  fontsize = 12)
matplotlib.rcParams.update({'font.size': 10})
plt.xlabel('Time Index')
plt.ylabel('Signal')

plt.plot(range(len(Data)), Data, 'b', lw = 2)
plt.plot(range(len(SmoothedData2)), SmoothedData2, 'r', lw = 2)
plt.xlim([0, DataLen])

fig.set_size_inches(10, 4)
plt.savefig('ItrSameBilateralRealSignal1.png')
