...
#f.write("Noise,SigD,SigI,PSNR\n")

for i in [1, 5, 10, 20, 30]:
    for j in [2.4, 2.5, 2.6]:
        for k in [2.9, 3.0, 3.1]:
            for l in range(5):

                TrueData, Data = buildsignal(i)
                SmoothedData1 = BilateralFilter(Data, j, k)
                SmoothedData2 = BilateralFilter(SmoothedData1, j, k)

                while np.sum((SmoothedData1 - SmoothedData2)**2) > 0.01*np.sum(SmoothedData2**2):
                    SmoothedData1 = BilateralFilter(SmoothedData2, j, k)
                    SmoothedData2 = BilateralFilter(SmoothedData1, j, k)

                f.write(str(i) + "," + str(j) + "," + str(k) + "," + str(psnr(SmoothedData2, TrueData)) + "\n")
